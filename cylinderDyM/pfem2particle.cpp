	...
	for(auto cell = dof_handlerU.begin(); cell != dof_handlerU.end(); ++cell){
		...
		for (unsigned int face_number=0; face_number < GeometryInfo<2>::faces_per_cell; ++face_number)
			if (cell->face(face_number)->at_boundary())
				for (unsigned int i=0; i < GeometryInfo<2>::vertices_per_face; ++i) movingNodes.at(cell->face(face_number)->vertex_index(i)).isBoundaryNode = true;
	}
			
	//3. Формирование контейнера указателей на ячейки сетки со связью со степенями свободы
	for(auto cell = dof_handlerP.begin(); cell != dof_handlerP.end(); ++cell) dofHandlerCells[cell->index()] = cell;
}
...
void pfem2Solver::reinterpolate_fields()
{
	...
	//согласно документации, Vector<double> инициализируется нулями
	Vector<double> newSolutionVx(dof_handlerVx.n_dofs());
	Vector<double> newSolutionVy(dof_handlerVy.n_dofs());
	Vector<double> newSolutionP(dof_handlerP.n_dofs());
	...
	for(std::map<int, meshNode>::iterator it = movingNodes.begin(); it != movingNodes.end(); ++it){
		...
		newCoords[0] += solutionU[it->second.dofUindexX];
		newCoords[1] += solutionU[it->second.dofUindexY];
		
		bool cellFound = false;
		Point<2> localCoords = mapping.transform_real_to_unit_cell(cell, newCoords);
		...
		for (unsigned int vertex=0; vertex<GeometryInfo<2>::vertices_per_cell; ++vertex){
			...
			newSolutionVx[it->second.dofScalarIndex] += shapeValue * solutionVx[dofCell->vertex_dof_index(vertex,0)];
			newSolutionVy[it->second.dofScalarIndex] += shapeValue * solutionVy[dofCell->vertex_dof_index(vertex,0)];
			newSolutionP[it->second.dofScalarIndex] += shapeValue * solutionP[dofCell->vertex_dof_index(vertex,0)];
		}//vertex
	}
	
	solutionVx = newSolutionVx;
	solutionVy = newSolutionVy;
	solutionP = newSolutionP;
}
...
void pfem2Solver::transform_grid()
{
	...
	//обновление координат в структурах узлов подвижной сетки (только после перемещения всех узлов сетки)
	for(std::map<int, meshNode>::iterator it = movingNodes.begin(); it != movingNodes.end(); ++it){
		it->second.coords[0] += solutionU[it->second.dofUindexX];
		it->second.coords[1] += solutionU[it->second.dofUindexY];
	}
}
