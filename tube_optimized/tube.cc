...
//	FEValues<2> feVx_values,
//				feVy_values,
//	            feP_values; 
						   
//	FEFaceValues<2> feVy_face_values,
//	                feP_face_values;
//						                                       
    const unsigned int   dofs_per_cellVx,
						 dofs_per_cellVy,
						 dofs_per_cellP;
	...
	FullMatrix<double>   local_matrixVx,
						 local_matrixVy,
						 local_matrixP;
						 
	Vector<double>       local_rhsVx,
						 local_rhsVy,
						 local_rhsP;
	
	std::vector<types::global_dof_index> local_dof_indicesVx,
	                                     local_dof_indicesVy,
										 local_dof_indicesP;
	...
	SparseMatrix<double> system_mVx, system_mVy, system_mP, systemVx, systemVy, systemP,
...
void tube::setup_system()
{
	...
	systemVx.reinit (sparsity_patternVx);
	system_mVx.reinit (sparsity_patternVx);
	systemCorrVx.reinit (sparsity_patternVx);
	system_mCorrVx.reinit (sparsity_patternVx);
	...
	systemVy.reinit (sparsity_patternVy);
	system_mVy.reinit (sparsity_patternVy);
	systemCorrVy.reinit (sparsity_patternVy);
	system_mCorrVy.reinit (sparsity_patternVy);
	...
	systemP.reinit (sparsity_patternP);
    ...
    
    {
		...
		for (; cell!=endc; ++cell,++number) {
			...
			for (unsigned int i=0; i<dofs_per_cellVx; ++i)
				for (unsigned int j=0; j<dofs_per_cellVx; ++j){
					systemVx.add (local_dof_indicesVx[i], local_dof_indicesVx[j], local_matrixVx(i,j));
	...
	
	{
		...
		for (; cell!=endc; ++cell) {
			...
			for (unsigned int i=0; i<dofs_per_cellVy; ++i)
				for (unsigned int j=0; j<dofs_per_cellVy; ++j){
					systemVy.add (local_dof_indicesVy[i], local_dof_indicesVy[j], local_matrixVy(i,j));
	...
	
	{
		...
		for (; cell!=endc; ++cell) {
			...
			for (unsigned int i=0; i<dofs_per_cellP; ++i)
				for (unsigned int j=0; j<dofs_per_cellP; ++j){
					systemP.add (local_dof_indicesP[i], local_dof_indicesP[j], local_matrixP(i,j));
	...
	{		
		...
		for (; cell!=endc; ++cell) {
			...
			for (unsigned int i=0; i<dofs_per_cellVx; ++i)
				for (unsigned int j=0; j<dofs_per_cellVx; ++j){
					systemCorrVx.add (local_dof_indicesVx[i],	local_dof_indicesVx[j],	local_matrixVx(i,j));
					//system_mCorrVx.add (local_dof_indicesVx[i],	local_dof_indicesVx[j],	local_matrixVx(i,j));
				}
		}//cell
	}//Vx correction
	...
	{	
		...
		for (; cell!=endc; ++cell) {
			...
			for (unsigned int i=0; i<dofs_per_cellVy; ++i)
				for (unsigned int j=0; j<dofs_per_cellVy; ++j){
					systemCorrVy.add (local_dof_indicesVy[i],	local_dof_indicesVy[j],	local_matrixVy(i,j));
					//system_mCorrVy.add (local_dof_indicesVy[i],	local_dof_indicesVy[j],	local_matrixVy(i,j));
				}
...
void tube::assemble_system()
{
	...
	FEFaceValues<2> feVy_face_values (feVy, face_quadrature_formula, update_values | update_quadrature_points | update_normal_vectors | update_JxW_values);
    FEFaceValues<2> feP_face_values (feP, face_quadrature_formula, update_values | update_quadrature_points  | update_normal_vectors | update_JxW_values);	
		
	for(int nOuterCorr = 0; nOuterCorr < 1; ++nOuterCorr){
		system_mVx.copy_from(systemVx);
		system_rVx=0.0;
		...
		std::map<types::global_dof_index,double> boundary_valuesVx2;
		VectorTools::interpolate_boundary_values (dof_handlerVx, 2, ConstantFunction<2>(0.0), boundary_valuesVx2);
		MatrixTools::apply_boundary_values (boundary_valuesVx2,	system_mVx, predictionVx, system_rVx);
...
*/
		...
		VectorTools::interpolate_boundary_values (dof_handlerVy, 2, ConstantFunction<2>(0.0), boundary_valuesVy2);
		...
		for (int n_cor=0; n_cor<1; ++n_cor){
			...
			system_mP.copy_from(systemP);
		
			std::map<types::global_dof_index,double> boundary_valuesP;
			VectorTools::interpolate_boundary_values (dof_handlerP, 4, ConstantFunction<2>(0.0), boundary_valuesP);
			MatrixTools::apply_boundary_values (boundary_valuesP, system_mP, solutionP, system_rP);
			...
			//Vx correction
			{
				...
				system_mCorrVx.copy_from(systemCorrVx);
			
				std::map<types::global_dof_index,double> boundary_valuesVx1;
				VectorTools::interpolate_boundary_values (dof_handlerVx, 1, ConstantFunction<2>(0.0), boundary_valuesVx1);
				MatrixTools::apply_boundary_values (boundary_valuesVx1, system_mCorrVx,	correctionVx, system_rVx);
										
				std::map<types::global_dof_index,double> boundary_valuesVx2;
				VectorTools::interpolate_boundary_values (dof_handlerVx, 2, ConstantFunction<2>(0.0), boundary_valuesVx2);
				MatrixTools::apply_boundary_values (boundary_valuesVx2, system_mCorrVx, correctionVx, system_rVx);

				std::map<types::global_dof_index,double> boundary_valuesVx3;
				VectorTools::interpolate_boundary_values (dof_handlerVx, 3, ConstantFunction<2>(0.0), boundary_valuesVx3);
				MatrixTools::apply_boundary_values (boundary_valuesVx3, system_mCorrVx, correctionVx, system_rVx);
			}//Vx correction
			...
			//Vy correction
			{
				...
				system_mCorrVy.copy_from(systemCorrVy);
			
				std::map<types::global_dof_index,double> boundary_valuesVy1;
				VectorTools::interpolate_boundary_values (dof_handlerVy, 1, ConstantFunction<2>(0.0), boundary_valuesVy1);
				MatrixTools::apply_boundary_values (boundary_valuesVy1, system_mCorrVy,	correctionVy, system_rVy);
										
				std::map<types::global_dof_index,double> boundary_valuesVy2;
				VectorTools::interpolate_boundary_values (dof_handlerVy, 2, ConstantFunction<2>(0.0), boundary_valuesVy2);
				MatrixTools::apply_boundary_values (boundary_valuesVy2, system_mCorrVy, correctionVy, system_rVy);

				std::map<types::global_dof_index,double> boundary_valuesVy3;
				VectorTools::interpolate_boundary_values (dof_handlerVy, 3, ConstantFunction<2>(0.0), boundary_valuesVy3);
				MatrixTools::apply_boundary_values (boundary_valuesVy3, system_mCorrVy, correctionVy, system_rVy);
			}//Vy
...
void tube::solveVx(bool correction)
{
	...
	if(correction){
		preconditioner.initialize(system_mCorrVx, 1.0);
		solver.solve (system_mCorrVx, correctionVx, system_rVx, preconditioner);
	}
	else {
		preconditioner.initialize(system_mVx, 1.0);
		solver.solve (system_mVx, predictionVx, system_rVx, preconditioner);
	}
...
void tube::solveVy(bool correction)
{
	...
	if(correction) {
		preconditioner.initialize(system_mCorrVy, 1.0);
		solver.solve (system_mCorrVy, correctionVy, system_rVy, preconditioner);
	}
	else {
		preconditioner.initialize(system_mVy, 1.0);
		solver.solve (system_mVy, predictionVy, system_rVy, preconditioner);
	}
