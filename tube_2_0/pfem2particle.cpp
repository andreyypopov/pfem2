...
void pfem2ParticleHandler::sort_particles_into_subdomains_and_cells()
{
	...
	//std::vector<std::unordered_multimap<int, pfem2Particle*>::iterator> sorted_particles;
		...
		
	{
      ...
      for (auto it = particles_out_of_cell.begin(); it != particles_out_of_cell.end(); ++it){
          ...
          const unsigned int closest_vertex = GridTools::find_closest_vertex_of_cell<2>(current_cell,(*particle).second->get_location());
...
*/
          ...
          //insert_particle((*particle).second, current_cell);
	...
	for (unsigned int i=0; i<particles_to_be_deleted.size(); ++i){
		...
		//remove_particle(particles_to_be_deleted[i]);
	}
	...
	for (; cell != endc; ++cell) {
		for(auto particleIndex = particle_handler.particles_in_cell_begin(cell); 
			...
			for (unsigned int vertex=0; vertex<GeometryInfo<2>::vertices_per_cell; ++vertex){
				...
				velocity_x[(*particleIndex).second->get_id()] += shapeValue * ( solutionVx(cell->vertex_dof_index(vertex,0)) - old_solutionVx(cell->vertex_dof_index(vertex,0)) );
				velocity_y[(*particleIndex).second->get_id()] += shapeValue * ( solutionVy(cell->vertex_dof_index(vertex,0)) - old_solutionVy(cell->vertex_dof_index(vertex,0)) );
			}//vertex
		}//particle
	}//cell
	...
	for (; cell != endc; ++cell) {
		...
		for (unsigned int vertex=0; vertex<GeometryInfo<2>::vertices_per_cell; ++vertex){
			...
			for (auto particleIndex = particle_handler.particles_in_cell_begin(cell); 
				...
				node_velocityX[cell->vertex_dof_index(vertex,0)] += shapeValue * velocity_x[(*particleIndex).second->get_id()];
				node_velocityY[cell->vertex_dof_index(vertex,0)] += shapeValue * velocity_y[(*particleIndex).second->get_id()];
