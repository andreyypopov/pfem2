...
void riverDischarge::build_grid ()
{
    ...
    const Point<2> top_right = Point<2> (100.0,10.0);
    
    std::vector< unsigned int > repetitions {200,20};
    ...
    for (; cell != endc; ++cell) {
        for (unsigned int face_number = 0; face_number<GeometryInfo<2>::faces_per_cell; ++face_number) {
            if(((std::fabs(cell->face(face_number)->center()(0)) < 1e-12) && (std::fabs(cell->face(face_number)->center()(1)) < 8)) || (std::fabs(cell->face(face_number)->center()(1)) < 1e-12)){
                cell->face(face_number)->set_boundary_id(1);
            }
            if ((std::fabs(cell->face(face_number)->center()(0)) < 1e-12) && (std::fabs(cell->face(face_number)->center()(1)) >= 8)) {
                cell->face(face_number)->set_boundary_id(0);
            }
            if (std::fabs(cell->face(face_number)->center()(0) - 100.0) < 1e-12) {
                cell->face(face_number)->set_boundary_id(2);
            }
            if (std::fabs(cell->face(face_number)->center()(1) - 10.0) < 1e-12) {
    ...
    for (; cell != endc; ++cell) {
        for (unsigned int face_number = 0; face_number<GeometryInfo<2>::faces_per_cell; ++face_number){            
            if(cell->face(face_number)->at_boundary() && cell->face(face_number)->boundary_id() == 0){
                for (unsigned int vert=0; vert<GeometryInfo<2>::vertices_per_cell; ++vert){
...
void riverDischarge::assemble_system()
{
    ...
    for(int nOuterCorr = 0; nOuterCorr < 1; ++nOuterCorr){
                    ...
                    if (cell->face(face_number)->at_boundary() && (cell->face(face_number)->boundary_id() == 2)){
                        ...
                        for (unsigned int q_point=0; q_point<n_face_q_points; ++q_point){
                            ...
                            for (unsigned int i=0; i<dofs_per_cellVx; ++i)
                            {
                                ...
                                //duydx += feVx_values.shape_grad(i,q_point)[0] * old_solutionVy(cell->vertex_dof_index(i,0));
                            }
                ...
                /*for (unsigned int face_number=0; face_number<GeometryInfo<2>::faces_per_cell; ++face_number)
                    if (cell->face(face_number)->at_boundary() && (cell->face(face_number)->boundary_id() == 3)){
                        ...
                        for (unsigned int q_point=0; q_point<n_face_q_points; ++q_point){
                            ...
                            for (unsigned int i=0; i<dofs_per_cellVx; ++i)
                            {
                                duxdy += feVy_values.shape_grad(i,q_point)[1] * old_solutionVx(cell->vertex_dof_index(i,0));
                                //duydx += feVx_values.shape_grad(i,q_point)[0] * old_solutionVy(cell->vertex_dof_index(i,0));
                            }
                            
                            for (unsigned int i=0; i<dofs_per_cellVx; ++i)
                            {
                                local_rhsVx(i) += mu * time_step * feVy_face_values.shape_value(i,q_point) * duxdy *
									feVy_face_values.normal_vector(q_point)[1] * feVy_face_values.JxW(q_point);
                            }
                        }
                    }*/
        ...
        VectorTools::interpolate_boundary_values (dof_handlerVx, 0, ConstantFunction<2>(2.0), boundary_valuesVx1);
        ...
        VectorTools::interpolate_boundary_values (dof_handlerVx, 1, ConstantFunction<2>(0.0), boundary_valuesVx3);
                ...
                for (unsigned int q_index=0; q_index<n_q_points; ++q_index) {
                    for (unsigned int i=0; i<dofs_per_cellVy; ++i) {
                        ...
                        for (unsigned int j=0; j<dofs_per_cellVy; ++j) {
                            ...
                            local_rhsVy(i) += (rho * Nj_vel * Ni_vel * old_solutionVy(cell->vertex_dof_index(j,0)) +
								time_step * g_y * 0.65 * Ni_vel * Nj_vel * (solutionSal(cell->vertex_dof_index(j,0)) - referenceSalinity)) * feVy_values.JxW (q_index);
                        }//j
                    }//i
                }//q_index
                ...
                for (unsigned int face_number=0; face_number<GeometryInfo<2>::faces_per_cell; ++face_number)
                    if (cell->face(face_number)->at_boundary() && (cell->face(face_number)->boundary_id() == 2)){
                        ...
                        for (unsigned int q_point=0; q_point<n_face_q_points; ++q_point){
                            ...
                            for (unsigned int i=0; i<dofs_per_cellVy; ++i)
                            {
                                duxdx += feVx_values.shape_grad(i,q_point)[0] * old_solutionVx(cell->vertex_dof_index(i,0));
                                //duydx += feVx_values.shape_grad(i,q_point)[0] * old_solutionVy(cell->vertex_dof_index(i,0));
                            }
                            
                            for (unsigned int i=0; i<dofs_per_cellVy; ++i)
                            {
                                local_rhsVy(i) += mu * time_step * feVy_face_values.shape_value(i,q_point) *(-2.0 * duxdx / 3.0)*
                                feVy_face_values.normal_vector(q_point)[1] * feVy_face_values.JxW(q_point);
                            }
                        }
                    }
        ...
        VectorTools::interpolate_boundary_values (dof_handlerVy, 0, ConstantFunction<2>(-0.1), boundary_valuesVy1);
        ...
        VectorTools::interpolate_boundary_values (dof_handlerVy, 1, ConstantFunction<2>(0.0), boundary_valuesVy4);
        ...
        for (int n_cor=0; n_cor<1; ++n_cor){
                ...
                for (; cell!=endc; ++cell) {
                    ...
                    for (unsigned int face_number=0; face_number<GeometryInfo<2>::faces_per_cell; ++face_number)
                        if (cell->face(face_number)->at_boundary() && (cell->face(face_number)->boundary_id() == 0)){
                            ...
                            for (unsigned int q_point=0; q_point<n_face_q_points; ++q_point)
                                for (unsigned int i=0; i<dofs_per_cellP; ++i)
                                    local_rhsP(i) -= rho / time_step * feP_face_values.shape_value(i,q_point) * 2.0 *
            ...
            VectorTools::interpolate_boundary_values (dof_handlerP, 3, ConstantFunction<2>(101325.0), boundary_valuesP1);
...
void riverDischarge::output_results(bool predictionCorrection)
{
    ...
    for(ParticleIterator<2> particleIndex = particle_handler.begin(); particleIndex != particle_handler.end(); ++particleIndex){
    ...
    for(ParticleIterator<2> particleIndex = particle_handler.begin(); particleIndex != particle_handler.end(); ++particleIndex){
        output2 << salinity[particleIndex->get_id()] << " ";
    }
    output2 << std::endl;
}
...
void riverDischarge::run()
{
    ...
    for (; time<=150; time+=time_step, ++timestep_number) {
