    ...
    for(unsigned int i = 0; i < quantities[0]; ++i){
        for(unsigned int j = 0; j < quantities[1]; ++j){
            ...
            for (unsigned int vertex=0; vertex<GeometryInfo<2>::vertices_per_cell; ++vertex){
                ...
                salinity[particle->get_id()] += shapeValue * solutionSal(cell->vertex_dof_index(vertex,0));
            }//vertex
        }
    }
}
    ...
    for(unsigned int i = 0; i < quantities[0]; i++){
        for(unsigned int j = 0; j < quantities[1]; j++){
            if(!particlesInParts[{i,j}]){
                ...
                res = true;
            } else if(particlesInParts[{i,j}] > MAX_PARTICLES_PER_CELL_PART){
				std::vector<ParticleIterator<2>> particlesToBeDeleted;
				...
				for(ParticleIterator<2> particleIndex = particle_handler.particles_in_cell(cell).begin(); particleIndex != particle_handler.particles_in_cell(cell).end(); ++particleIndex){
					...
					if(num_x == i && num_y == j){
						particlesToBeDeleted.push_back(particleIndex);
						if(--particlesInParts[{i,j}] == MAX_PARTICLES_PER_CELL_PART) break;
					}					
				}
				
				for(unsigned int k = 0; k < particlesToBeDeleted.size(); ++k) particle_handler.remove_particle(particlesToBeDeleted[k]);
...
void pfem2Solver::distribute_particle_velocities_to_grid() //перенос скоростей частиц на узлы сетки
{
    ...
    Vector<double> node_velocityX, node_velocityY,node_salinity;
    ...
    for (; cell != endc; ++cell) {
		for (ParticleIterator<2> particleIndex = particle_handler.particles_in_cell(cell).begin();
			...
			for (unsigned int vertex=0; vertex<GeometryInfo<2>::vertices_per_cell; ++vertex){
                ...
                node_salinity[cell->vertex_dof_index(vertex,0)] += shapeValue * salinity[particleIndex->get_id()];
    ...
    //принудительное обнуление солености в граничных узлах, иначе решение смазывается и на входе в область порождаются частицы уже с ненулевой соленостью
