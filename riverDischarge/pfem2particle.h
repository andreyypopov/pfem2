...
public:
    ...
    virtual void initialize_node_solutions() = 0;
    ...
    Vector<double> old_solutionVx, old_solutionVy, old_solutionP,solutionSal;        //!< Вектор решения на предыдущем шаге по времени (используется для вычисления разности с текущим и последующей коррекции скоростей частиц)

    std::map<int, double> velocity_x, velocity_y,salinity;
...
protected:
    ...
    std::set<unsigned int> boundaryDoFNumbers;	//номера степеней свободы, соленость в которых необходимо принудительно обнулять
